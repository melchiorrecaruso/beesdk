...
begin
  ...
  begin
    ...
    if not ((0 < SymbolCodeLen) and (SymbolCodeLen <=
      aCodeLenTree.LookupBitLength)) then
      raise EAbInternalInflateError.Create(
...
begin
  ...
  while ((Symbol <> 256) {and (ExtraBitCount <> 15)}) do
  begin
    ...
    begin
      ...
      if (not aDeflate64) and (Symbol = 285) then
      begin
        Length := 258;
        {$IFOPT C+}
        aInStrm.DiscardBits(SymbolCodeLen);
...
begin
  ...
  try {finally}
    try {except}
      ...
      if Assigned(Helper.OnProgressStep) then
      begin
      ...
      begin
        ...
        {$ENDIF}
        DecryptStrm :=
          TAbDfDecryptStream.Create(aSource,
          Helper.CheckValue, Helper.Passphrase);
      ...
      OutWindow := TAbDfOutputWindow.Create(aDest,
        UseDeflate64, UseCRC32, Helper.PartialSize, TestOnly, Log);
      ...
      on E: EAbPartSizedInflate do
      begin
        {nothing, just swallow the exception}
        Result := 0;
      end;
      on E: EAbAbortProgress do
      begin
        {nothing, just swallow the exception}
        Result := 0;
      end;
      on E: EAbInternalInflateError do
      begin
        if (Log <> nil) then
