...
begin
  ...
  for Symbol := 3 to pred(aCodeCount) do
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      else if (StoredSize < BitCount) then
      begin
        EncodeLZStreamStored(aFinalBlock, aStream, aBitStrm,
          (StoredSize div 8) - 4, aLog);
        Exit;
    ...
    Code := (LitCodeCount - 257) + ((DistCodeCount - 1) shl 5) +
  ...
  try
    ...
    begin
      ...
      {note: the test for whether a stream is binary or not is to
             ...
             PKZIP 'n' option.}
      if TestForBinary and (LZ77Stream.StoredSize = 1024) then
      begin
        if (aHelper.PKZipOption = 'n') then
          if (LZ77Stream.LitBuckets^[0] = 0) then
          begin
      ...
      begin
        ...
        {if, by this point, we're storing up a match, check to see
         ...
         for a lazy match}
        if (PrevMatch.maLen >= aHelper.MaxLazyLength) then
        begin
          {$IFDEF UseLazyMatchLogging}
          if (aLog <> nil) then
...
begin
  ...
  try
    ...
    begin
      ...
      if Assigned(aHelper.OnProgressStep) then
      begin
...
begin
  ...
  try {finally}
    try {except}
      ...
      on E: EAbInternalDeflateError do
      begin
        {$IFDEF UseLogging}
        if (Log <> nil) then
