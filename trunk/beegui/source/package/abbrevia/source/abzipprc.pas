...
procedure AbZip(Sender: TAbZipArchive; Item: TAbZipItem; OutStream: TStream);
...
{ ========================================================================== }
procedure DoDeflate(Archive: TAbZipArchive; Item: TAbZipItem;
  OutStream, InStream: TStream);
{!!.02 Added }
const
...
{ ========================================================================== }
procedure DoStore(Archive: TAbZipArchive; Item: TAbZipItem;
  OutStream, InStream: TStream);
var
  CRC32:  longint;
  Percent: longint;
...
begin
  ...
  while (DataRead <> 0) and not Abort do
  begin
    {report the progress}
    if Assigned(Archive.OnProgress) then
    begin
...
var
  ...
  begin
    ...
    if not (InStream is TFileStream) or (InStream is TAbSpanStream) then
    begin
...
{$ENDIF}
  ...
  begin
    ...
    begin
      Item.GeneralPurposeBitFlag :=
        Item.GeneralPurposeBitFlag or AbFileIsEncryptedFlag or AbHasDataDescriptorFlag;
    end;
    ...
    begin                                      {!!.01}
      ...
      case ZipArchive.CompressionMethodToUse of
        ...
        begin
          ...
          if TempOut.Size > InStream.Size then
          begin { store item instead }
...
{$ENDIF}
    ...
    {  OEMToAnsi(Buff, Buff);  }
    UncompressedStream := TFileStream.Create(StrPas(Buff), fmOpenRead or
      fmShareDenyWrite);
