...
type
  ...
  public
    ...
    property CompressionMethod: byte Read FGZHeader.CompMethod
      Write FGZHeader.CompMethod;

    property ExtraFlags: byte {Default: 2}
      Read FGZHeader.XtraFlags Write FGZHeader.XtraFlags;

    property Flags: byte Read FGZHeader.Flags Write FGZHeader.Flags;
    ...
    property HeaderCRC: word Read GetHeaderCRC;

    property IsEncrypted: boolean Read GetIsEncrypted;

    property HasExtraField: boolean Read GetHasExtraField;

    property HasFileName: boolean Read GetHasFileName;

    property HasFileComment: boolean Read GetHasFileComment;

    property HasHeaderCRC: boolean Read GetHasHeaderCRC;

    property IsText: boolean Read GetIsText Write SetIsText;

    property GZHeader: TAbGzHeader Read FGZHeader Write FGZHeader;

    property IncludeHeaderCrc: boolean Read FIncludeHeaderCrc Write FIncludeHeaderCrc;
  ...
  public
    ...
    property CRC: longint Read GetGzCRC;
    property FileSize: longint Read GetFileSize;
    property TailCRC: longint Read FTail.CRC32;
    property TailSize: longint Read FTail.ISize;
  end;
  ...
  private
    ...
    FGZItem: TAbArchiveList;
    { item in Gzip (only one, but need polymorphism of class)}
  ...
  public {methods}
    ...
    property TarAutoHandle: boolean Read FTarAutoHandle Write SetTarAutoHandle;

    property IsGzippedTar: boolean Read GetIsGzippedTar Write FIsGzippedTar;
...
begin
  ...
  Result := (Header.ID1 = AB_GZ_HDR_ID1) and (Header.ID2 = AB_GZ_HDR_ID2) and
...
begin
  ...
  try
    ...
    { check id fields and if deflated (only handle deflate anyway)}
    if VerifyHeader(GHlp.FItem.FGZHeader) then
    begin
...
begin
  ...
  if (DataRead = SizeOf(TAbGzHeader)) and VerifyHeader(GZH) then
  begin
    FItem.GZHeader := GZH;
    Result := True;
  end;
...
procedure TAbGzipArchive.ExtractItemAt(Index: integer; const NewName: string);
var
...
begin
  ...
  begin
    SwapToGzip;
    if Index > 0 then
      Index := 0; { only one item in a GZip}
...
procedure TAbGzipArchive.ExtractItemToStreamAt(Index: integer; aStream: TStream);
var
        ...
        case CurItem.Action of
          ...
          begin
            ...
            try
              ...
              DoArchiveItemProgress(CurItem, 0, Abort);
              // end !! 0.7
              ...
              // !!.07 Notify Item Progress
              DoArchiveItemProgress(CurItem, 100, Abort);
              // end !! 0.7
            finally
              WorkingStream.Free;
            end;
          ...
          begin
            try
              ...
              begin
                ...
                UncompressedStream :=
                  TFileStream.Create(CurItem.DiskFileName, fmOpenRead or
                  fmShareDenyWrite);
                try
                  GetDir(0, SaveDir);
                  try {SaveDir}
                    ...
                    CurItem.ExternalFileAttributes :=
                      AbFileGetAttr(CurItem.DiskFileName);
