...
{ Contains:
  ...
  Fist release:
    ...
    v1.0 build 0013 - 2013.09.20 by Melchiorre Caruso.
...
type
  ...
  TConfirm = (arcOk, arcCancel, arcQuit);

  /// archive events

  TCommentArcEvent = procedure (var CommentAs: string;
    var Confirm: TConfirm) of object;

  TCommentItemEvent = procedure (Item: TArchiveItem;
    var CommentAs: string; var Confirm: TConfirm) of object;

  TDeleteItemEvent = procedure(Item: TArchiveItem;
    var Confirm: TConfirm) of object;

  TExtractItemEvent = procedure(Item: TArchiveItem;
    var ExtractAs: string; var Confirm: TConfirm) of object;

  TRenameItemEvent = procedure(Item: TArchiveItem;
    var RenameAs: string; var Confirm: TConfirm) of object;

  TSendMessageEvent = procedure(const Msg: string) of object;

  TSendPercentageEvent = procedure(Percentage: longint) of object;

  TUpdateEvent = procedure(SearchRec: TCustomSearchRec;
    var UpdateAs: string; var Confirm: TConfirm) of object;
  ...
  private
    ...
    // events
    FOnCommentArc: TCommentArcEvent;
    FOnCommentItem: TCommentItemEvent;
    FOnDeleteItem: TDeleteItemEvent;
    FOnExtractItem: TExtractItemEvent;
    FOnRenameItem: TRenameItemEvent;
    FOnSendMessage: TSendMessageEvent;
    FOnSendPercentage: TSendPercentageEvent;
  ...
  private
    function  DoCommentArc: TConfirm;
    function  DoCommentItem(Item: TArchiveItem): TConfirm;
    function  DoDeleteItem (Item: TArchiveItem): TConfirm;
    function  DoExtractItem(Item: TArchiveItem): TConfirm;
    function  DoRenameItem (Item: TArchiveItem): TConfirm;
    ...
    procedure DoSendPercentage;
    function  DoUpdateItem (Item: TCustomSearchRec): TConfirm;
  ...
  public
    property OnCommentArc: TCommentarcEvent read FOnCommentArc write FOnCommentArc;
    property OnCommentItem: TCommentItemEvent read FOnCommentItem write FOnCommentItem;
    property OnDeleteItem: TDeleteItemEvent read FOnDeleteItem write FOnDeleteItem;
    property OnExtractItem: TExtractItemEvent read FOnExtractItem write FOnExtractItem;
    property OnRenameItem: TRenameItemEvent read FOnRenameItem write FOnRenameItem;
    ...
    property OnSendPercentage: TSendPercentageEvent read FOnSendPercentage write FOnSendPercentage;
    property OnUpdateItem: TUpdateEvent read FOnUpdateItem write FOnUpdateItem;
  end;

function CheckIntegrityMethodToStr(Method: longint): string;
...
begin
  ...
  begin
    DoSendMessage(Format(cmOpening, [aArchiveName]));
  ...
  begin
    DoSendMessage(Format(cmCreating, [aArchiveName]));
    FArchiveName := aArchiveName;
  end;
end;
...
begin
  ...
  begin
    ...
    case Item.FTag of
      aitDecode:    DoSendMessage(Format(cmSwapping, [Item.FFileName]));
      aitDecAndUpd: DoSendMessage(Format(cmDecoding, [Item.FFileName]));
    end;
...
begin
  ...
  begin
    ...
    Tester.OnSendPercentage          := OnSendPercentage;
    Tester.OnSendMessage           := OnSendMessage;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DoSendMessage(Format(cmSplitting, [FCentralDirectory.Items[I].FileName]));
...
procedure TArchiver.DoSendMessage(const Message: string);
begin
  if Assigned(FOnSendMessage) then
    FOnSendMessage(Message);
end;

procedure TArchiver.DoSendPercentage;
begin
  if Assigned(FOnSendPercentage) then
    FOnSendPercentage(Round((FProcessedSize/FTotalSize)*100));
end;

function TArchiver.DoCommentArc: TConfirm;
var
  CommentAs: string;
begin
  Result := arcCancel;
  if Assigned(FOnCommentArc) then
  begin
    CommentAs := FCentralDirectory.FComment;
    FOnCommentArc(CommentAs, Result);
    case Result of
      arcOk: begin
        FCentralDirectory.FComment := CommentAs;
        FIsNeededToRun  := TRUE;
...
function TArchiver.DoCommentItem(Item: TArchiveItem): TConfirm;
var
  CommentAs: string;
begin
  Result := arcCancel;
...
function TArchiver.DoExtractItem(Item: TArchiveItem): TConfirm;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Item := FCentralDirectory.Items[I];
      case Item.FTag of
        aitUpdate: DoSendMessage(Format(cmExtracting, [Item.FExternalFileName]));
        aitDecode: DoSendMessage(Format(cmDecoding,   [Item.FFileName]));
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Item := FCentralDirectory.Items[I];
      case Item.FTag of
        aitUpdate: DoSendMessage(Format(cmTesting,  [Item.FFileName]));
        aitDecode: DoSendMessage(Format(cmDecoding, [Item.FFileName]));
      end;
...
function TArchiver.DoRenameItem(Item: TArchiveItem): TConfirm;
var
  RenameAs: string;
begin
  Result := arcCancel;
  ...
  begin
    ...
    if Result in [arcOk, arcCancel] then
      DoCommentItem(Item);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Item := FCentralDirectory.Items[I];
      case Item.FTag of
        aitNone:   DoSendMessage(Format(cmCopying,  [Item.FileName]));
        aitUpdate: DoSendMessage(Format(cmRenaming, [Item.FileName]));
        else       SetExitStatus(esCaseError);
      end;
...
function TArchiver.DoDeleteItem(Item: TArchiveItem): TConfirm;
begin
  Result := arcCancel;
...
begin
  ...
  begin
    ...
    begin
      ...
      Item := FCentralDirectory.Items[I];
      case Item.FTag of
        aitUpdate:    DoSendMessage(Format(cmDeleting, [Item.FileName]));
        aitDecAndUpd: DoSendMessage(Format(cmDeleting, [Item.FileName]));
      end;
    ...
    begin
      ...
      case Item.FTag of
        aitNone:   DoSendMessage(Format(cmCopying,  [Item.FileName]));
        aitDecode: DoSendMessage(Format(cmEncoding, [Item.FileName]));
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // check method
      CurrentItem.FCheckMethod         := ExtractCheckIntegrityMethod   (FCheckParams);
      CurrentItem.FCheckMethodAux      := ExtractCheckIntegrityAuxMethod(FCheckParams);
...
function TArchiver.DoUpdateItem(Item: TCustomSearchRec): TConfirm;
var
  I: longint;
  UpdateAs: string;
begin
  ...
  begin
    ...
    case Result of
      arcOk: begin
        ...
        DoCommentItem(FCentralDirectory.Items[I]);
      end;
      arcCancel: begin
        I := IndexOf(UpdateAs);
        if I <> -1 then
...
begin
  ...
  begin
    ...
    begin
      ...
      case Item.FTag of
        aitNone:      DoSendMessage(Format(cmCopying,  [Item.FileName]));
        aitAdd:       DoSendMessage(Format(cmAdding,   [Item.FileName]));
        aitUpdate:    DoSendMessage(Format(cmUpdating, [Item.FileName]));
        aitDecode:    DoSendMessage(Format(cmEncoding, [Item.FileName]));
        aitDecAndUpd: DoSendMessage(Format(cmUpdating, [Item.FileName]));
      end;
