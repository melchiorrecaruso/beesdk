...
type
  ...
  TSwitch = (
    ...
    swtCI,  // swtCI   Set check integrity parameters
    swtE,   // swtE    Set encryption parameters
    swtL,   // swtL    Set current archive layer
  ...
  protected
    ...
    procedure ProcessSwitchCI(var S: string);
    procedure ProcessSwitchE(var S: string);
    procedure ProcessSwitchI(var S: string);
    procedure ProcessSwitchL(var S: string);
  ...
  public
    ...
    property SwitchCI: string read FSwitchCI;
    property SwitchE: string read FSwitchE;
    property SwitchL: longint read FSwitchL;
  ...
  function ExtractCheckIntegrityMethod(Params: string): longword;
  function ExtractCheckIntegrityAuxMethod(Params: string): longword;
...
procedure TCommandLineParser.ProcessSwitchCI(var S: string);
begin
  Delete(S, 1, 4);
  FSwitchCI := FSwitchCI + S + char(0);
  // check ...
  ExtractCheckIntegrityMethod(FSwitchCI);
  ExtractCheckIntegrityAuxMethod(FSwitchCI);

  if Command in [cmdA] then
  begin
    if ExitStatus = esNoError then
      Include(FSwitches, swtCI);
  end else
...
procedure TCommandLineParser.ProcessSwitchL(var S: string);
var
  I: longint;
begin
  Delete(S, 1, 3);
  if TryStrToInt(S, FSwitchL) = FALSE then
    SetExitStatus(esCmdLineLError);

  if FCommand in [cmdA, cmdD, cmdE, cmdL, cmdQ, cmdR, cmdT, cmdX] then
  begin
    if ExitStatus = esNoError then
      Include(FSwitches, swtL);
  end else
    SetExitStatus(esCmdLineLError);
end;
  ...
  if ParamCount > 1 then
    ...
    begin
      ...
      begin
        ...
        if Pos('-CI/',   UpCase(S)) = 1 then ProcessSwitchCI (S) else
        if Pos('-E/',    UpCase(S)) = 1 then ProcessSwitchE  (S) else
        if Pos('-I/',    UpCase(S)) = 1 then ProcessSwitchI  (S) else
        if Pos('-L/',    UpCase(S)) = 1 then ProcessSwitchL  (S) else
