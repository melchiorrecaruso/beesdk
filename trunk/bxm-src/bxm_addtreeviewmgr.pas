...
begin
  ...
  for j := 1 to Min(Length(Str1), Length(Str2)) do
    ...
    begin
      if AnsiCompareFileName(IncludeTrailingBackSlash(Result), Result) <> 0 then
      begin
...
begin
  ...
  while AnsiCompareFileName(RootValue, Value) <> 0 do
  begin
    I    := Spin;
    Spin := I + 1;
    if Spin = I then
...
begin
  ...
  begin
    Result :=
      FFilePath.Strings[Index] + FFileName.Strings[Index] +
...
begin
  ...
  begin
    F := FFilePath.Strings[I] + FFileName.Strings[I] +
      FFileMask.Strings[I];
...
begin
  if (Value > -1) and (AnsiCompareFileName(GetRootValue(Value),
    GetRootValue(FSpin)) <> 0) then
  begin
    FSpin := Value;
    Update;
  end;
...
procedure Register;
begin
  RegisterComponents('BX Manager', [TAddTreeViewMgr]);
end;
