...
begin
  Result := -1;
  for I := 0 to Count - 1 do
    if AnsiCompareFileName(TArchiveItem(Items[I]).FileName, FileName) = 0 then
    begin
      if AnsiCompareFileName(TArchiveItem(Items[I]).FilePath, FilePath) = 0 then
      begin
        Result := I;
        Break;
      end;
...
begin
  ...
  begin
    case SortColumn of
      0: Result  := AnsiCompareFileName(AI1.FileName,     AI2.FileName);
      1: Result  :=                    (AI1.FileSize    - AI2.FileSize);
      2: Result  :=                    (AI1.FilePacked  - AI2.FilePacked);
      3: Result  :=                    (AI1.FileRatio   - AI2.FileRatio);
      4: Result  := AnsiCompareFileName(AI1.FileType,     AI2.FileType);
      5: Result  := Round              (AI1.FileTime    - AI2.FileTime);
      6: Result  :=                    (AI1.FileAttr    - AI2.FileAttr);
      7: Result  := AnsiCompareText    (AI1.FileMethod,   AI2.FileMethod);
      8: Result  := AnsiCompareText    (AI1.FilePassword, AI2.FilePassword);
      9:
      begin
      ...
      10: Result := AnsiCompareFileName(AI1.FilePath,      AI2.FilePath);
...
begin
  ...
  begin
    for I := 0 to FFolders.Count - 1 do
      if AnsiCompareFileName(FFolder, TArchiveItem(FFolders.Items[I]).FilePath) = 0 then
      begin
        SetData(Items.Add, FFolders.Items[I]);
      end;

    for I := 0 to FFiles.Count - 1 do
      if AnsiCompareFileName(FFolder, TArchiveItem(FFiles.Items[I]).FilePath) = 0 then
      begin
        SetData(Items.Add, FFiles.Items[I]);
      end;
    ...
    begin
      ...
      begin
        if AnsiCompareFileName(FFolderBoxSign + FFolder, FFolderBox.Items[I]) = 0 then
        begin
          J := I;
          Break;
        end;
...
begin
  ...
  if AnsiCompareFileName(FFolder, D) <> 0 then
    SetFolder(D)
  else
    Result := False;
end;
...
begin
  ...
  begin
    Node := TArchiveItem(Items[I].Data);
    if FileNameMatch(Node.FileName, Mask, Value) then
    begin
...
var
  Node: TArchiveItem;
begin
  if AnsiCompareFileName(AArchiveName, FFileName) <> 0 then
  begin
    FFolder := '';
  end;
...
begin
  if Assigned(SmallImages) and (SmallImages.ClassType = TIconList) then
    ...
    begin
      ...
      begin
        if AnsiCompareFileName(FFolderBoxSign + AFolder, FFolderBox.Items[J]) = 0 then
        begin
          K := J;
          Break;
        end;
...
procedure Register;
begin
  RegisterComponents('BX Manager', [TArchiveListView]);
end;

initialization

  {$i bxm_archivelistviewmgr.lrs }
