...
begin
  ...
  if Assigned(aOnProgress) and (aStreamSize > 0) then
  begin
...
begin
  ...
  begin
    ...
    FStream.Seek(FStream.Position + FFakeCount -
      (FBufEnd - FBuffer) + (FBufPos - FBuffer) -
...
begin
  ...
  begin
    ...
    if ((FBufEnd - FBufPos) < sizeof(TAb32bit)) then
      ...
      TempBuffer := PAb32bit(FBufPos)^;
    Result := Result + ((TempBuffer and AbExtractMask[BitsToGo]) shl
      FBitsLeft);
  end;
...
begin
  ...
  if ((FBufEnd - FBufPos) < sizeof(TAb32bit)) then
    ...
    TempBuffer := PAb32bit(FBufPos)^;
  Result := Result + ((TempBuffer and AbExtractMask[BitsToGo]) shl FBitsLeft);
end;
...
begin
  ...
  begin
    ...
    FBitBuffer := PAb32bit(FBufPos)^;
    Inc(FBufPos, sizeof(TAb32bit));
    Result := Result + ((FBitBuffer and AbExtractMask[BitsToGo]) shl
      FBitsLeft);
    FBitBuffer := FBitBuffer shr BitsToGo;
    FBitsLeft := 32 - BitsToGo;
  end;
...
begin
  ...
  begin
    ...
    for i := 0 to pred(BytesToRead) do
    begin
...
{$ENDIF}
{--------}
function TAbDfLZStream.AddLenDist(aLen: integer;
  aDist: integer): boolean;
var
  LenSymbol: integer;
  DistSymbol: integer;
  CurPos: PAnsiChar;
...
begin
  ...
  while (CurPos < StrmEnd) do
  begin

    {if the next item is a literal...}
    if boolean(CurPos^) then
    begin
...
begin
  ...
  for i := 257 to 279 do
    Inc(Result, FLitBuckets^[i] * (7 +
      dfc_LitExtraBits[i - dfc_LitExtraOffset]));
  for i := 280 to 284 do
    Inc(Result, FLitBuckets^[i] * (8 +
      dfc_LitExtraBits[i - dfc_LitExtraOffset]));
...
begin
  ...
  if (((FLitCount + FDistCount) and $1FFF) = 0) then
  begin
    Result := (FDistCount < FLitCount) and (lzsGetApproxSize <
      (FStoredSize div 2));
    if Result then
      Exit;
  end;
  ...
  Result := (FDistCount >= 32768) or ((FLitCount + FDistCount) >= 32768);
end;
