...
type
  ...
  TAbArchiveAction =
    (aaFailed, aaNone, aaAdd, aaDelete, aaFreshen, aaMove, aaCopy, aaReplace,
    aaStreamAdd, aaRename);
  TAbProcessType =
    (ptAdd, ptDelete, ptExtract, ptFreshen, ptMove, ptReplace, ptFoundUnhandled);
  TAbLogType =
    (ltAdd, ltDelete, ltExtract, ltFreshen, ltMove, ltReplace, ltStart,
    ltFoundUnhandled);
  TAbErrorClass =
...
{ date and time stuff }
const
  Date1900 {: LongInt} = $0001AC05;
  {Julian day count for 01/01/1900 -- TDateTime Start Date}
...
const
  ...
  AB_FPERMISSION_GENERIC =
    AB_FPERMISSION_OWNERREAD or AB_FPERMISSION_OWNERWRITE or
    AB_FPERMISSION_OWNEREXECUTE or AB_FPERMISSION_GROUPREAD or
...
{$IFDEF MSWINDOWS }
  function GetLocalDiskFree(const Path: string):
{$IFDEF VERSION4} Int64 {$ELSE}
  longint
{$ENDIF}
  ;
  var
    SectorsPerCluster, BytesPerSector, NumberOfFreeClusters, TotalNumberOfClusters:
{$IFDEF VERSION4} Cardinal {$ELSE}
    longint
{$ENDIF}
    ;
    Succeeded: BOOL;
  ...
  begin
    ...
    Succeeded := GetDiskFreeSpace(DrvBuf, SectorsPerCluster,
  ...
  function GetRemoteDiskFree(const Path: string):
{$IFDEF VERSION4} Int64 {$ELSE}
  longint
{$ENDIF}
  ;
...
{$IFDEF VERSION4} TLargeInteger {$ELSE}
    longint
{$ENDIF}
    ;
    Succeeded: BOOL;
...
{$IFDEF MSWINDOWS }
  ...
  case AbGetPathType(ArchiveName) of
    ptNone, ptRelative: { if path is relative or bad }
      Size := -1; { fail }
    ptAbsolute:
    begin {path is absolute}
      if Pos('\\', ArchiveName) = 1 then
      begin  {path is UNC; must refer to network }
        { check OS version }
        if OSOK then
        begin
...
{$ENDIF LINUX}
  if (Size < -1) or (Size > MaxLongInt) then
  begin
    Result := MaxLongInt;
  end
  else
...
begin
  ...
  Result := DirMatch and AbPatternMatch(ExtractFileName(FileName),
    1, ExtractFileName(FileMask), 1);
end;
...
begin
  ...
  begin
    try
      while (Found = 0) do
      begin
        if (SR.Name <> AbThisDir) and (SR.Name <> AbParentDir) and
...
begin
  ...
  begin
    J := I;
    while (I <= Length(FileMask)) and (FileMask[I] <> AbPathSep) do
      Inc(I);
    MaskPart := Trim(Copy(FileMask, J, I - J));
    if (I <= Length(FileMask)) and (FileMask[I] = AbPathSep) then
      Inc(I);
...
begin
  ...
  try
    ...
    if SubPaths.Count > 0 then
      for i := 0 to pred(SubPaths.Count) do
      begin
    ...
    Found  := FindFirst(Search, faReadOnly +
      faHidden + faSysFile + faArchive, SR);
    if Found <> 0 then
...
begin
  ...
  begin
    if (CompareStr(Pattern, AbDosAnyFile) = 0) or
      (CompareStr(Pattern, AbUnixAnyFile) = 0) then
    begin
      Result := True;
      Exit;
    end;
  ...
  begin
    ...
    begin
      if ((Pattern[iPat] = '?') and (Length(Source) <> iSrc - 1)) or
        (Pattern[iPat] = Source[iSrc]) then
      begin
        Inc(iPat);
        Inc(iSrc);
      end
...
begin
  if V2 > 16384000 then
  begin  {Possible LongInt overflow}
...
begin
  ...
  Result := Unix0Date + (UnixTime div SecondsInDay) + EncodeTime(Hrs, Mins, Secs, 0);
end;
...
begin
  ...
  begin
    ...
    {$IFDEF DefeatWarnings}{$IFDEF Version6} {$WARN SYMBOL_PLATFORM ON} {$ENDIF}
{$ENDIF}
    FileClose(f);
  end;
end;
...
begin
 ...
 { -------------------------------------------------------------------------- }
 {!!.01 -- Added }
function AbFileGetSize(const aFileName: string):{$IFDEF MSWINDOWS}
...
procedure AbSetSpanVolumeLabel(Drive: AnsiChar; VolNo: integer);
begin
  AbWriteVolumeLabel(Format(AB_SPAN_VOL_LABEL, [VolNo]), Drive);
end;
