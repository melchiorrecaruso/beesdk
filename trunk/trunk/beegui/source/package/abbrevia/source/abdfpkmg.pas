...
{--------}
procedure QSS(aList: PPkgNodeList; aFirst: integer;
  aLast: integer);
var
  L, R:  integer;
  Pivot: integer;
...
begin
  ...
  begin
    ...
    begin
      repeat
        Dec(R);
      until (aList^[R]^.pnWeight <= Pivot);
      repeat
        Inc(L);
      until (aList^[L]^.pnWeight >= Pivot);
      if (L >= R) then
        Break;
      Temp := aList^[L];
      aList^[L] := aList^[R];
...
begin
  ...
  try
    ...
    begin
      ...
      while (OrigInx < OrigListCount) and (PkgInx < PkgListCount) do
      begin
        if (OrigList^[OrigInx]^.pnWeight <= PkgList^[PkgInx]^.pnWeight) then
        begin
    ...
    for i := 0 to (OrigListCount * 2) - 3 do
    begin
      Node := MergeList^[i];
