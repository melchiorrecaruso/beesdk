...
{$IFDEF VERSION3} override; {$ENDIF}
    {-set the stream size}

    property MaxMemToUse: longint Read vmsMaxMemToUse Write vmsSetMaxMemToUse;
    {-maximum memory to use for data before swapping to disk}
    property SwapFileDirectory: string Read vmsSwapFileDir Write vmsSwapFileDir;
  end;
...
begin
  ...
  begin
    ...
    for Inx := pred(vmsPageList.Count) downto 0 do
    begin
      Page := PvmsPage(vmsPageList[Inx]);
      if (Page^.vpStmOfs >= NewSize) then
        Dispose(Page)
      else if (Page^.vpStmOfs < NewSize) then
      begin
...
begin
  ...
  for Inx := 0 to pred(vmsMaxPages) do
  begin
    Page := PvmsPage(vmsPageList[Inx]);
    if (Page^.vpLRU < OldestLRU) then
    begin
      OldestInx  := Inx;
...
begin
  ...
  if (vmsPageList.Count < 4) then
  begin
    L := vmsPageList.Count;
    for M := 0 to pred(vmsPageList.Count) do
    begin
  ...
  try
    ...
    if (vmsPageList.Count < vmsMaxPages) then
    begin
      New(Page);
      CreatedNewPage := True;
    end
    ...
    else {vmsMaxPages <= vmsPageList.Count}
    begin
      {find the oldest page}
      vmsFindOldestPage(OldestPageNum, Page);
      {if it is dirty, write it out to the swap file}
...
var
  ...
  SeekResult: longint;
begin
  if (vmsSwapHandle = 0) or (aPage^.vpStmOfs >= vmsSwapFileSize) then
  begin
  ...
  else {there is something to be read from the swap file}
  begin
