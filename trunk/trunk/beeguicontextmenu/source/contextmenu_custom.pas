...
type
  ...
  public
    // Abstract methods
    procedure MeasureMenuItem(IdCmd: UINT; var Width: UINT; var Height: UINT);
      virtual; abstract;
    procedure DrawMenuItem(Menu: HMENU; IdCmd: UINT; DC: HDC;
...
function TCustomContextMenu.SEIInitialize(pidlFolder: PItemIDList;
...
function TCustomContextMenu.QueryContextMenu(Menu: HMENU;
...
begin
  ...
  if LastUserCmdId = 0 then
    ...
    Result := MakeResult(SEVERITY_SUCCESS, FACILITY_NULL, LastUserCmdId -
      idCmdFirst + 1);
end;
...
function TCustomContextMenu.GetCommandString(idCmd, uFlags: UINT;
...
function TOwnerDrawContextMenu.HandleMenuMsg(uMsg: UINT;
  wParam, lParam: integer): HResult;
var
  ...
  case uMsg of
    ...
    begin
      try
        ...
        // Call abstract method
        DrawMenuItem(dis.hwndItem, dis.itemID, dis.hDC, dis.rcItem,
          TOwnerDrawState(LongRec(dis.itemState).Lo));
      except
        Result := E_FAIL;
      end;
    end;
...
procedure Initialize(ComClass: TComClass; ClassID: TGUID; ClassName: string);
begin
  TComObjectFactory.Create(ComServer, ComClass, ClassID, ClassName,
