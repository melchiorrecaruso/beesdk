...
type
  ...
  private
    Ratio : integer;
  public
    function UpdateAInfo(const ArcName: string; AInfo: TArchiveListDetails): boolean;
    function UpdateFInfo(FInfo: TArchiveListItem): boolean;
  end;
...
uses
  ...
  begin
    ...
    {$I beegui_infofrm.inc}
    InfoFrm_Storage.Restore;
  end;

  function TInfoFrm.UpdateAInfo(const ArcName: string; AInfo: TArchiveListDetails): boolean;
  begin
    Result := True;
  ...
  function TInfoFrm.UpdateFInfo(FInfo: TArchiveListItem): boolean;
  begin
    Result := True;
    if Assigned(FInfo) then
    try
      if TArchiveListItem(FInfo).FileSize = 0 then
        Ratio  := 0
      else
        Ratio := Round(100 * (TArchiveListItem(FInfo).FilePacked / TArchiveListItem(FInfo).FileSize));

      InfoFrm_FName.Caption         := TArchiveListItem(FInfo).FileName;
      InfoFrm_FVersion.Caption      := TArchiveListItem(FInfo).FileVersion;
      InfoFrm_FSize.Caption         := SizeToStr(TArchiveListItem(FInfo).FileSize);
      InfoFrm_FPacked.Caption       := SizeToStr(TArchiveListItem(FInfo).FilePacked);
      InfoFrm_FRatio.Caption        := IntToStr(Ratio) + '%';
      InfoFrm_FAttribute.Caption    := AttrToStr(TArchiveListItem(FInfo).FileAttr);
      InfoFrm_FPassword.Caption     := TArchiveListItem(FInfo).FilePassword;
      InfoFrm_FMethod.Caption       := TArchiveListItem(FInfo).FileMethod;
      InfoFrm_FModified.Caption     := DateTimeToStr(FileDateToDateTime(TArchiveListItem(FInfo).FileTime));
    except
      Result := False;
    end;
