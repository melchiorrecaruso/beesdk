...
var
  ...
  begin
    ...
    if FileExists(FileName + '.bkp') then
      try
        Stream := TFileStream.Create(FileName + '.bkp', fmOpenRead);
      except
        Stream := nil;
      end
    else
  ...
  begin
    ...
    begin
      ...
      BeeModeller_SetTableParameters(Modeller, @Person.Genome[1]);

      DictionaryLevel := CurrentAge div 2000 + 1;
      BeeModeller_SetDictionaryLevel(Modeller, DictionaryLevel);

      if Assigned(DrawDictionaryLevel) then
        DrawDictionaryLevel(DictionaryLevel);

      NulWriter.Seek(0, 0);
      BeeRangeEnc_StartEncode(Coder);
      ...
      begin
        ...
        while Loop <> 0 do
        begin
          Inc(Readed, BeeModeller_Encode(Modeller,
      ...
      BeeRangeEnc_FinishEncode(Coder);
    end;
  ...
  begin
    ...
    begin
      Writeln('This is BxOpt 1.0 utility (C) 2002-2013 Andrew Filinsky.');
      Writeln('This program calculates parameters for Bee 1.0 archiver utility,');
      Writeln('then better compression ratio will be available by using them.');
      Writeln;
      Writeln('Usage: BxOpt <Ext> [<Options>]');
      Writeln;
