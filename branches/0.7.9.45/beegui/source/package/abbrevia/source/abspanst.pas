...
type
  ...
  public
    ...
    property Threshold: longint Read FThreshold Write FThreshold default 0;
    ...
    property FreeSpace: longint Read GetSpace;
...
begin
  Result := 0;                                                           {!!.01}
  if FIgnoreSpanning then
  begin                                          {!!.01}
    Result := FStr.Read(Buffer, Count);                                  {!!.01}
  end                                                                    {!!.01}
  else
  begin
    if (Count > 0) and (FStr.Position = FStr.Size) then
    begin { need next span }
      if not Assigned(FOnRequestImage) then
        exit;
      FStr.Free;
      FStr := nil;
      Inc(FSpanNumber);
      ...
      if Valid and not FCancelled then
      begin
...
begin
  ...
  begin
    ...
    if FMediaType = mtLocal then
    begin                  { media not removeable }
      if (FThreshold = 0) then
      begin                    { not local span }
        TotalWritten := FStr.Write(Buffer, Count);      { write buffer }
      end

      else
      begin                                        { it's a local span }
        if GetSpace > Count then
        begin                  { there's room on }
          { the local span }
        ...
        begin                                      { not enough room }
          ...
          begin           { still data in Buffer }
            DoRequestNewMedia{('Media Full')};            { skip to next medium }

            CurWritten :=                               { write as much as }
              FStr.Write(LocalPtr^, Least([Count - TotalWritten, GetSpace]));
            { there's room for }
...
begin
  ...
  begin
    ...
    if (NewPos < 0) then
    begin  { past beginning of current stream }

      { request previous media }
      if not Assigned(FOnRequestImage) then
        exit;
      Dec(FSpanNumber);
      FOnRequestImage(Self, FixSpanNumber(FSpanNumber), FImageName, FCancelled);
      {!!.01}
    ...
    if (NewPos > FStr.Size) then
    begin { past end of current stream }
      { request next media }
      if not Assigned(FOnRequestImage) then
        exit;
      Dec(FSpanNumber);
    ...
    else { offset is within current stream }
    begin
...
begin
  ...
  if ((Mode and fmCreate) = fmCreate) or ((Mode and fmOpenWrite) = fmOpenWrite) then
    FSpanMode := smWriting
...
begin
  ...
  if FBytesAvail < -1 then
    FBytesAvail := High(FBytesAvail);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if ValidateImageName(NewName) then
        begin
          Mode := FFileMode;
          ValidName := True;
          FImageName := NewName;
    ...
    raise EAbZipstreamFull.Create; {!!.05 [ 753982 ],[714944] }
end;
...
begin
  if FMediaType = mtRemoveable then
  begin
