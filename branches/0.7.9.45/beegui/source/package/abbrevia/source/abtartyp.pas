...
type
  ...
  public
    property DevMajor: integer Read GetDevMajor Write SetDevMajor;
    property DevMinor: integer Read GetDevMinor Write SetDevMinor;
    property GroupID: integer Read GetGroupID Write SetGroupID;
    property GroupName: string Read GetGroupName Write SetGroupName;
    property LinkFlag: AnsiChar Read FTarHeader.LinkFlag Write FTarHeader.LinkFlag;
    property LinkName: string Read GetLinkName Write SetLinkName;
    property Mode: longint Read GetExternalFileAttributes
...
begin
  ...
  while (i < aLen) and (Oct[i] in ['0'..'7']) do
  begin
...
begin
  ...
  try
    ...
    { verify check sum }
    TarChkSum := OctalToInt(TarItem.FTarHeader.ChkSum,
      sizeof(TarItem.FTarHeader.ChkSum));
    TarHead.ChkSum := AB_TAR_CHKBLANKS;
    if (CalcTarHeaderChkSum(TarHead) = TarChkSum) or
      VerifyHeader(TarItem.FTarHeader) then
      Result := atTar
    else
      Result := atUnknown;
  finally
...
begin
  Result := (Size + (AB_TAR_RECORDSIZE - 1)) and not (AB_TAR_RECORDSIZE - 1);
end;
...
begin
  ...
  while TarF.Position < TarF.Size do
  begin
    { if it's a file }
    if TarH.LinkFlag in [AB_TAR_LF_OLDNORMAL, AB_TAR_LF_NORMAL] then
    begin
...
begin
  ...
  while (DataRead = SizeOf(TAbTarHeaderRec)) and not
    (FTarHeader.LinkFlag in [AB_TAR_LF_OLDNORMAL, AB_TAR_LF_NORMAL]) do
  begin
    FStream.Seek(-SizeOf(TAbTarHeaderRec), soFromCurrent);
...
begin
  ...
  while (DataRead = SizeOf(TAbTarHeaderRec)) and (StrLen(FTarHeader.Name) > 0) and
    not (FTarHeader.LinkFlag in [AB_TAR_LF_OLDNORMAL, AB_TAR_LF_NORMAL]) do
  begin
    FStream.Seek(-SizeOf(TAbTarHeaderRec), soFromCurrent);
...
procedure TAbTarArchive.ExtractItemToStreamAt(Index: integer; aStream: TStream);
var
...
begin
  ...
  try {TarHelp}
    ...
    { while more data in Tar }
    while (FStream.Position < FStream.Size) and ItemFound do
    begin
      Item := TAbTarItem.Create;
      try  {Item}
        Item.LoadTarHeaderFromStream(FStream);
        {if it's a file}
        if Item.LinkFlag in [AB_TAR_LF_OLDNORMAL, AB_TAR_LF_NORMAL] then
        begin
          {create new Item}
          Item.Action := aaNone;
          FItemList.Add(Item);
...
var
  InTarHelp, OutTarHelp: TAbTarStreamHelper;
  Abort: boolean;
  i: integer;
    ...
    try {NewStream/OutTarHelp}
      ...
      begin
        ...
        FStream := TAbSpanStream.Create(FArchiveName, fmOpenWrite or
          fmShareDenyWrite, mtLocal, FSpanningThreshold);
