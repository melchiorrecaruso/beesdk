...
type
  ...
  TPopulations = class(TList)
    ...
    procedure Live;
    /// Имитировать размножение одной из особей одной из популяций
    procedure MarkToRecalculate;
    /// Отметить все особи всех популяций как не оцененые
    destructor Destroy; override;
    /// Уничтожить набор популяций вместе с содержимым
  end;
...
begin
  Result := False;
  for I := 2 to SizeOf(Genome) do
    if Genome[I] <> Person.Genome[I] then
      exit;
  Result := True;
end;
...
begin
  ...
  if Count > 0 then
    TPerson(First).MarkToRecalculate;
end;
...
begin
  Result := True;
  for I := 0 to Count - 1 do
    if Person.IsEqual(TPerson(List[I])) then
      exit;
  Result := False;
end;
...
begin
  ...
  if FileExists(FileName + '.Err') then
    Stream := TFileReader.Create(FileName + '.Err', fmOpenRead)
  else
  if FileExists(FileName) then
    Stream := TFileReader.Create(FileName, fmOpenRead);
  /// Поток создан?
  if Stream <> nil then
  begin
...
begin
  ...
  begin /// Популяция пуста?
    /// Построить новую особь случайным образом...
    Person := TPerson.Create;
    MainForm.StatusBar.Panels.Items[0].Text := 'Generate random creature...';
  end else if TPerson(Population1.First).Cost = 0 then
  begin /// Первая особь популяции не оценена?
  ...
  begin
    ...
    MainForm.Label_DictionaryLevelValue.Caption :=
      Format('%d (~%d Mb)', [DictionaryLevel,
  ...
  WriteText(MainForm.App.SrcName + '.log.txt', Format('%d' + #9,
    [TPerson(Population1.First).Cost]));
  /// Вывести отчет обо всех популяциях...
  if CurrentPopulation = 14 then
    WriteText(MainForm.App.SrcName + '.log.txt',
      Format('| %5d turn, -d%d, %d jumps (%1.1f%%).' + Cr,
...
begin
  ...
  begin
    ...
    begin
      Delete(S, 1, 2);
      Cfg.Selector('\main');
      Cfg.CurrentSection.Values['Method'] :=
        IntToStr(Max(1, Min(StrToInt(S), 3)));
    end else
    ...
    begin
      Delete(S, 1, 2);
      Cfg.Selector('\main');
      Cfg.CurrentSection.Values['Dictionary'] :=
        IntToStr(Max(0, Min(StrToInt(S), 9)));
    end else
...
begin
  ...
  begin
    if TPopulation(World.List[I]).Count = 0 then
      Continue;
    WriteText('Collect.txt',
      Format('%d:', [TPerson(TPopulation(World.List[I]).First).Genome[1]]) +
      #9 + Format('%d', [TPerson(TPopulation(World.List[I]).First).Cost]));
    if TPerson(TPopulation(World.List[I]).First).Genome[1] < 3 then
    begin
      WriteText('Collect.txt', Cr);
      Continue;
    end else if (Levels.Count > 0) and
      (TPerson(TPopulation(World.List[I]).First).Cost > TPerson(Levels.Last).Cost) then
    begin
      WriteText('Collect.txt', Cr);
      Continue;
    end else
  ...
  begin
    Cfg.Selector('\m1');
    Cfg.CurrentSection.Values[Ext + '.Size'] :=
      IntToStr(TPerson(Levels.First).Cost);
    Cfg.PutData(Ext, TPerson(Levels.First).Genome,
      SizeOf(TPerson(Levels.First).Genome));
    Cfg.Selector('\m2');
    Cfg.CurrentSection.Values[Ext + '.Size'] :=
      IntToStr(TPerson(Levels.List[Levels.Count div 2]).Cost);
    Cfg.PutData(Ext, TPerson(Levels.List[Levels.Count div 2]).Genome,
      SizeOf(TPerson(Levels.First).Genome));
    Cfg.Selector('\m3');
    Cfg.CurrentSection.Values[Ext + '.Size'] := IntToStr(TPerson(Levels.Last).Cost);
    Cfg.PutData(Ext, TPerson(Levels.Last).Genome,
      SizeOf(TPerson(Levels.First).Genome));
  end;
end;
...
begin
  ...
  if FileExists(SelfPath + CfgName) then
    Cfg.LoadFromFile(SelfPath + CfgName);
  ...
  begin
    Cfg.Selector('\m1');
    ReduceSection(Cfg.CurrentSection);
    Cfg.Selector('\m2');
    ReduceSection(Cfg.CurrentSection);
    Cfg.Selector('\m3');
    ReduceSection(Cfg.CurrentSection);
...
begin
  ...
  begin
    ...
    MainForm.StatusBar.Panels.Items[0].Text :=
      'Parameters will be re-estimated at next run...';
    Exit;
  end;
  ...
  begin
    ...
    /// Выдать status bar ...
    MainForm.StatusBar.Panels.Items[0].Text :=
      'All "' + SrcName + '.dat" merged. Run again to continue...';
    Exit;
  end;
  ...
  begin
    MainForm.StatusBar.Panels.Items[0].Text := 'Configuration file was maked.';
    Exit;
  end;
  if SrcName = '' then
  begin
    MainForm.StatusBar.Panels.Items[0].Text := 'Folder is not selected.';
    Exit;
  end;
  if Headers.Count = 0 then
  begin
    MainForm.StatusBar.Panels.Items[0].Text := 'No Files for Optimization.';
    Exit;
  end;
  ...
  repeat
    DrawLevelProgress;
    World.Live;
    if not MainForm.NeedToClose then
      World.Save(SrcName + '.dat');
...
begin
  ...
  MainForm.Label_PercentOfImprovementsValue.Caption :=
    Format('%1.3f%%', [World.Improvements / (World.CurrentAge + 1) * 100]);
  ...
  if TPopulation(World.List[World.CurrentPopulation]).Count > 0 then
    MainForm.Label_PackedSizeValue.Caption :=
      Format('%d', [TPerson(TPopulation(World.List[World.CurrentPopulation]).First).Cost])
  else
    MainForm.Label_PackedSizeValue.Caption := '?';

  MainForm.RxTrayIcon1.Hint :=
    Format('%d variants, %1.3f%% improvements.', [World.CurrentAge,
    World.Improvements / (World.CurrentAge + 1) * 100]);
end;
